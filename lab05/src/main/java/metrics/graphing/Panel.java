...
public class Panel extends JPanel {
    ...
    private DataDisplayStrategy dataDisplayStrategy;

    private Plot plot;

    public Panel(Plot plot) {
        this.plot = plot;
        this.dataDisplayStrategy = new AllPointsStrategy();
    }

    public void setPlot(Plot plot) {
        this.plot = plot;
    }
    public List<Double> calculateDisplayedData(List<Double> inputData) {
        return dataDisplayStrategy.calculateData(inputData);
    }
    
    public void setDataDisplayStrategy(DataDisplayStrategy strategy) {
        this.dataDisplayStrategy = strategy;
    }
    ...
    private void paintData(Graphics2D g) {
        ...
        List<Double> displayedData = calculateDisplayedData(emitterData);
        
        // List<Double> displayedData = emitterData;
        // TODO change data based on statistic
       
        double x = (double) (getWidth() - 2 * marg) / (displayedData.size() - 1);
        
        double scale = (double) (getHeight() - 2 * marg)
                / (Math.max(Math.abs(getMinValue(displayedData)), Math.abs(getMaxValue(displayedData))) * 2);
        paintGridLines(g, displayedData);
        ...
        for (int i = 0; i < displayedData.size(); i++) {
            double x1 = marg + i * x;
            double y1 = getHeight() / 2 + displayedData.get(i) * scale;
