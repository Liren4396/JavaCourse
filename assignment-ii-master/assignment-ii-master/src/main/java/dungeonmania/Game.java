...
public class Game {
    ...
    public int getSpawners() {
        return this.getMap().getEntities(ZombieToastSpawner.class).size();
    }
    ...
    public Game tick(Direction movementDirection) {
        ...
        if (player.hasSceptre() != null) {
            Sceptre sceptre = player.hasSceptre();
            sceptre.use(this);
        }
        tick();
        return this;
    ...
    public Game build(String buildable) throws InvalidActionException {
        ...
        if (buildables.contains("midnight_armour") && this.map.getEntities(ZombieToast.class).size() >= 1) {
            throw new InvalidActionException(String.format("midnight_armour cannot be built"));
        }
