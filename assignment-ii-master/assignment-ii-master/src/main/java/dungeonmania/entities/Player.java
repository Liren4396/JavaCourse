...
public class Player extends Entity implements Battleable {
    ...
    public void increaseDeafedEnemy() {
        this.defeatedEnemy++;
    }

    public int getDefeatedSpawners() {
        return defeatedSpawner;
    }

    public void increaseDefeatedSpawner() {
        this.defeatedSpawner++;
    }
    ...
    public boolean pickUp(Entity item) {
        if (item instanceof Key && getInventory().count(Key.class) > 0) {
            return false;
        }
    ...
    public void triggerNext(int currentTick) {
        ...
        if (inEffective instanceof InvincibilityPotion) {
            state.toInvincible(this);
        } else {
            state.toInvisible(this);
        }
