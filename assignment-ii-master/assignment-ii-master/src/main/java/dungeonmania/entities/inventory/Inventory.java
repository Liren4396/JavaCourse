...
public class Inventory {
    ...
    public List<String> getBuildables() {
        ...
        if ((wood >= 2 && (treasure >= 1 || keys >= 1)) || (wood >= 2 && sunstones >= 1)) {
            result.add("shield");
        }
    ...
    public InventoryItem checkBuildCriteria(Player p, boolean remove, boolean forceShield, EntityFactory factory) {
        ...
        List<Sword> swords = p.getInventory().getEntities(Sword.class);
        ...
        if (wood.size() >= 1 && arrows.size() >= 3 && !forceShield) {
        ...
        } else if (wood.size() >= 2 && sunstones.size() >= 1) {
            if (remove) {
                items.remove(wood.get(0));
                items.remove(wood.get(1));
            }
            ...
            if (remove) {
                if (keys.size() >= 1 && sunstones.size() >= 1) {
                    ...
                    items.remove(sunstones.get(0));
                    return factory.buildSceptre();
                } else if (treasure.size() >= 1 && sunstones.size() >= 1) {
                    items.remove(sunstones.get(0));
                    items.remove(wood.get(0));
                    items.remove(treasure.get(0));
                    return factory.buildSceptre();
